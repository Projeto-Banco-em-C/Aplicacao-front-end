...
//FORMATA VALOR 4 CADAS DECIMAIS
function mValor4(v){
    v=v.replace(/\D/g,"");
...
$(document).ready(function(){
   ...
   $(document).on('keyup', '.mValor4',function(){
	    var valor = $(this).val();
	    valor=valor.replace(/[^01234567890,]+/g,'');
	    $(this).val(valor);
	});
	$(document).on('blur', '.mValor4',function(){
	    var valor = $(this).val();
	    var float = gdfunctions.getFloatValue(valor);
	    float = parseFloat(float.toFixed(4));
	    valor = mValor4(gdfunctions.getStringValue(float,4));
	    $(this).val(valor);   
	});
